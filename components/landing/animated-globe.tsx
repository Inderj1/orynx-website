...
// Glyph body (the ASCII sphere texture from the original hero), on a 0.15 rad lat/lon grid.
const GLYPHS = "░▒▓█▀▄▌▐│─┤├┴┬╭╮╰╯";
const GLYPH_GRID: { lat: number; lon: number }[] = [];
for (let th = 0.12; th < Math.PI - 0.12; th += 0.15) {
  // Widen the longitude step toward the poles so glyphs stay evenly spaced instead of knotting.
  const step = 0.15 / Math.max(0.3, Math.sin(th));
  for (let ph = 0; ph < Math.PI * 2 - step / 2; ph += step) GLYPH_GRID.push({ lat: Math.PI / 2 - th, lon: ph });
}
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (body > 0) {
        ...
        ctx.strokeStyle = rgba(COLORS.ink, 0.4 * body);
        ctx.lineWidth = 1;
        ctx.stroke();
      }

      // Glyph body: the original ASCII sphere texture, now riding the same camera as the rings.
      // Fades in with the tilt so the flat logo visibly gains mass and becomes a world.
      if (body > 0) {
        ctx.font = `${Math.max(9, Math.round(S / 66))}px ${mono}`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        // Slow secondary drift so the texture never reads as a rigid grid.
        const drift = t * 0.35 * RAD;
        for (const g of GLYPH_GRID) {
          const p = project(g.lat, g.lon + spin * 0.6 + drift, e, 0.985, R, cx, cy);
          const depth01 = (p.d + 1) / 2;
          const a = (0.06 + 0.5 * depth01 * depth01) * body;
          if (a < 0.03) continue;
          ctx.fillStyle = rgba(COLORS.ink, a);
          ctx.fillText(GLYPHS[Math.floor(depth01 * (GLYPHS.length - 1))], p.sx, p.sy);
        }
      }

      // Meridians every 30°, kept faint under the glyph body. Cut at ±78° so the pole never knots.
      if (body > 0) {
        for (let m = 0; m < 12; m++) {
          const lon = m * 30 * RAD + spin;
          const pts = [];
          for (let k = 0; k <= 26; k++) {
            const lat = (-78 + (156 * k) / 26) * RAD;
            pts.push(project(lat, lon, e, 1, R, cx, cy));
          }
          strokePolyline(pts, COLORS.ink, 0.09 * body, 0.03 * body, 1);
        }
      }
      ...
      RINGS.forEach((ring, i) => {
        ...
        ringPts.push(pts);
        strokePolyline(pts, ring.color, 0.95, 0.22, 2.2);
      ...
      if (body > 0.3) {
        RINGS.forEach((ring, i) => {
          ...
          for (let k = Math.max(0, kMax - 1); k <= kMax; k++) {
            ...
            const r = st.stage === 3 ? 4 + 2 * Math.min(1, st.lift) : 4;
            ctx.beginPath();
            ctx.arc(p.sx, p.sy, r, 0, Math.PI * 2);
